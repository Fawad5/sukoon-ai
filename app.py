...
# In a professional app, you would store these in a database.
# Here we initialize the 'credentials' in session state so they persist during the session.
    ...
    st.session_state.credentials = {
        'usernames': {
            'user123': {
                ...
                'name': 'Sukoon User',
                'password': '123' # In production, use stauth.Hasher(['123']).generate()
            }
        }
    }
...
if st.session_state["authentication_status"]:
    ...
    if user_input:
        if any(word in user_input.lower() for word in ["suicide", "hurt", "die", "khudkushi", "marna"]):
            ...
            with st.spinner("Finding sukoon for you..."):
                ...
                Format: ENG_PART: [English] VERSE_PART: [Hadith/Ayah] URDU_PART: [Urdu]"""
    ...
    st.caption("Sukoon AI | Guided by Faith, Powered by AI")
